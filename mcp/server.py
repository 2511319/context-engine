...
from core.merger import apply_token_budget, dedup_overlaps
from core.pg import PgClient
...
from core.resolver import apply_rules, heuristic_module, rule_from_cfg
...
class Health:
    ...
    def neo4j_ok(uri: Optional[str], user: Optional[str], password: Optional[str]) -> bool:
        if not uri or not user or not password or GraphDatabase is None:
            return False
        try:
...
class MCPServer:
    ...
    def _tool_get_context(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        rules_cfg = (cfg.raw.get("routing", {}) or {}).get("rules", [])
        rules = [r for r in (rule_from_cfg(x) for x in (rules_cfg or [])) if r]
        module = apply_rules(task, rules)

        neo_modules = list_modules_neo4j(self.env.neo4j_uri or "", self.env.neo4j_user or "", self.env.neo4j_pass or "", project)

        pg_client = PgClient(self.env.pg_dsn or "")
        if not module:
            module = heuristic_module(task, pg_client, project, neo_modules) or ""
        ...
        code_cands, doc_cands = apply_token_budget(code_cands, doc_cands, budget=token_budget_limit)
        token_budget_snapshot = self._token_budget_snapshot(
            code_cands, doc_cands, pre_budget_code, pre_budget_docs, token_budget_limit
        )
    ...
    def _tool_search_raw(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        code, docs = hybrid_search(
            PgClient(self.env.pg_dsn or ""),
            project,
            query,
            self.env.openai_key,
            weights,
            rcand,
            str(retr.get("lexical_config", "simple")),
        )
    ...
    def _store_feedback(self, params: Dict[str, Any], label: int) -> Dict[str, Any]:
        ...
        task_fp = compute_task_fp(task)
        with psycopg.connect(self.env.pg_dsn, autocommit=True) as conn:  # type: ignore[arg-type]
            with conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO dp_feedback(project, task_fp, uri, label) VALUES (%s,%s,%s,%s)",
                    (project, task_fp, uri, label),
                )
    ...
    def _tool_explain_plan(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not Health.pg_ok(self.env.pg_dsn):
            raise ToolExecutionError("postgres unavailable", {"component": "postgres"})
        query = "SELECT plan_id, detail FROM plan_log"
        where: list[str] = []
        args: list[Any] = []
        if plan_id:
        ...
        with psycopg.connect(self.env.pg_dsn, autocommit=True) as conn:  # type: ignore[arg-type]
            with conn.cursor() as cur:
                ...
                row = cur.fetchone()
                if not row and last:
                    cur.execute("SELECT plan_id, detail FROM plan_log ORDER BY ts DESC LIMIT 1")
                    row = cur.fetchone()
                if not row:
                    raise ToolExecutionError("plan not found", {"plan_id": plan_id})
                return {"plan_id": row[0], "detail": row[1]}
